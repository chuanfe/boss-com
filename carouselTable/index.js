...
class carouseTable {
  ...
  initInteraction() {
    ...
    self.container.off('mouseenter').off('mouseleave').on('mouseenter', () => {
      self.removeTimer();
    }).on('mouseleave', function (e) {
      const c = e.toElement || e.relatedTarget;
  ...
  initRank() {
    ...
    const rowCount = cfg.global.rowCount;
  ...
  // 获取展示的数据
  getData(dataPool, startIndex, rowCount) {
    const filterData = [];
    if (startIndex > dataPool.length - 1) { // 截取位置超出数组长度
      if (this.config.global.animation.mode === 'bottom' && dataPool.length > 0) {
    ...
    for (let i = startIndex; i < startIndex + rowCount; i++) {
      if (dataPool[i] && undefined !== dataPool[i]) {
        const k = _.cloneDeep(dataPool[i]);
        k.index = i + 1;
        filterData.push(k);
  ...
  setStartIndex(dataPool, startIndex, step) {
    startIndex += step;
    startIndex > dataPool.length - 1 && (startIndex = 0);
    this.startIndex = startIndex;
  }
  initFill(dataPool, startIndex) {
    const cfg = this.config;
    const series = cfg.series;
    const columnNameList = this.columnNameList || [];
    const $rows = this.container.find(`.row-content:lt(${cfg.global.rowCount})`);
    $rows.each((index, rowEl) => {
      const item = dataPool[startIndex + index];
      const $row = $(rowEl);
      if (item) {
        $row.find('.cell-content').each((cIndex, cellEl) => {
          const i = columnNameList[cIndex];
  ...
  fillCell(a, b) {
    const self = this;
    const cfg = this.config;
    const startIndex = a.index;
    const rowBg1 = cfg.row.backgroundColor1;
    const rowBg2 = cfg.row.backgroundColor2;
    ...
    $rows.each((rowIndex, rowEl) => {
      const rowObj = a.data[rowIndex];
      const realIndex = startIndex + rowIndex + 1;
      if (rowObj) {
        if (Object.keys(rowObj).length === 0 && cfg.global.ifRowHidden) {
          return $(rowEl).css('opacity', 0);
        }
        $(rowEl).css('opacity', 1);
        $(rowEl).find('.cell-content').each(function (cellIndex, cellEl) {
          const i = self.columnNameList[cellIndex];
          const j = self.config.series[cellIndex].dataType;
          let renderStr = '';
          if (j === 'img') {
            renderStr = `<img src="${rowObj[i] || ''}" style="width: ${cfg.series[cellIndex].widthPercent}%; height:100%;" />`;
          } else if (rowObj[i] || +rowObj[i] === 0) {
            renderStr = rowObj[i];
          } else {
            renderStr = '-';
          }
          const $mt = $(this).find('.marquee-text');
          const $mtSpan = $(this).find('.marquee-text').find('.marquee-text-span');
          const indexNum = `${realIndex}-${cellIndex}`;
          $mtSpan.html(renderStr);
        ...
        $(rowEl).css('background-color', realIndex % 2 === 0 ? rowBg2 : rowBg1);
  ...
  // 溢出文本滚动
  textRun(textTimerId, $mt, cellIndex) {
  ...
  // 动画
  animation() {
    const aniEl = this.config.global.animation.mode === 'bottom' ?
  ...
  // 轮播
  loop() {
    const { global: { animation: { mode, duration } } } = this.config;
    let rowCount = this.config.global.rowCount;
    this.removeTimer();
    if (this.dataPool) {
      let step;
      if (mode === 'bottom') {
      ...
      if (this.isInit) {
        ...
        const data = this.getData(this.dataPool, this.startIndex, rowCount);
        this.fillCell(data);
        this.animation();
  ...
  render(_data, config) {
    ...
    if (data) {
      ...
      if (!cfg.global.isLoop || cfg.global.animation.singleStop && data.length <= cfg.global.rowCount) {
        ...
        this.isInit = true;
        if (this.dataPool) {
          const datas = this.getData(this.dataPool, 0, cfg.global.rowCount);
          this.fillCell(datas, !self.isLoop);
        }
      } else { // 轮播状态
        if (this.isInit) {
          this.removeTimer();
          if (this.dataPool) {
            const datas = this.getData(this.dataPool, this.startIndex, cfg.global.rowCount);
            this.fillCell(datas, !self.isLoop);
          }
          this.loop();
          this.isInit = false;

...
class TextScroll {
  ...
  render(data, config) {
    ...
    const tpl = `
      <div class="boss-text-scroll">
        <div class="text-scroll-container">
          <span class="text-scroll-content"></span>
        </div>
      </div>
    `;
    this.container.html(tpl);
    const $tsc = this.container.find('.text-scroll-content');
    if (data[0].url || url) {
      $tsc.html(xss(`<a href="${data[0].url || url}" ${ifBlank ? 'target="_blank"' : ''}>${_content}</a>`, {
        whiteList: {
          a: ['href', 'title', 'target']
        }
      }));
    ...
    if (onOff) {
      this.container.find('.boss-text-scroll')
        .marquee(resetScrollConfig);
    }
  }
